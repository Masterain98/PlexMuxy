> **Source:** https://www.matroska.org/technical/ordering.html
...
When using Medium Linking, chapters are used to reference other Segments to play in a given order (#medium-linking). A Segment containing these linked Chapters does not require a `Track` Element or a `Cluster` Element.
