...
def test_subset_font_keeps_requested_characters_and_preserves_original_family_name(tmp_path: Path) -> None:
    ...
    font = TTFont(source)
    # Add a localized (Chinese) family record on top of the English one to mimic
    # real fonts that ship several family-name variants, e.g. GDI-packed names.
    ...
    # written into the font. The subset preserves EVERY original family-name record so
    # players/libass match it exactly like a full embedded font -- including packed
    # variants a style might reference.
    ...
    try:
        ...
        # Both the English and localized family names survive subsetting.
        assert "Original Family" in families
        assert "原始字体" in families
