...
def format_plan(plan: MuxPlan) -> list[str]:
    ...
    if plan.subtitle_tracks:
        ...
        for track in plan.subtitle_tracks:
            default = "true" if track.default_track else "false"
            forced = "true" if track.forced_track else "false"
            lines.append(f"  + {track.path.name}")
            lines.append(f"    language: {track.track_name} / {track.mkv_language} / {track.ietf_language}")
            lines.append(f"    default: {default}")
            lines.append(f"    forced: {forced}")
            lines.append(f"    reason: {track.match_reason}")
